...
min_minutes = 30
while True:
    valve, opened, pressure, been, minutes = max(paths, key = lambda x: x[2] + heuristic(x[1], x[4]))
    paths.remove((valve, opened, pressure, been, minutes))
    pressure += sum(valves[i][0] for i in opened)
    minutes -= 1
    if minutes == 0:
        print(valve, opened, pressure, len(paths))
        break
